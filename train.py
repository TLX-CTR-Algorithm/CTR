...
def train_model(batch_size=config.batch_size):
    ...
    global_step = 0
    while 1 == 1:
        batch_inputs, batch_lables = sample.genbatch( inputs, lables, batch_size=config.batch_size)
        #进行1个epoch的训练
        for step in range(len(inputs)/batch_size):
            ...
            with tf.Session() as sess:
                global_step, logits, loss = sess.run(dnnmodel.global_step, dnnmodel.train_step, dnnmodel.loss, feed_dict=feed_dict)
