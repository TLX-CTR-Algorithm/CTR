...
def create_feature(data_path, datadir, filename = None):
    ...
    device_id_cnt, device_ip_cnt, user_cnt, user_hour_cnt = scan(data_path + filename)
    ...
    data_df['day']=np.round(data_df.hour % 10000 / 100).astype('int') #生成日期，某天
        ...
        with open(datafile, 'r') as f:
            for line in f:
                ...
                for i in range(0, self.num_feature):#估计是除去首列id列，循环处理前面13个连续特征
                    ...
                    if val != '':
                        val = int(val) #向下取整
                        if val > continous_clip[i]:#若大于分割点，则以分割点为值，也就是去除异常值，超过95%为异常点
                            val = continous_clip[i]
                        self.min[i] = min(self.min[i], val) #与该位置的值比较，取较小值，找出该位置的最小值
                        self.max[i] = max(self.max[i], val) #与该位置的值比较，取较大值，找出该位置的最大值

    def gen(self, idx, val):#val为值，idx是连续型变量的位置，从0开始
        if val == '':
            return 0.0
        val = float(val)
        return (val - self.min[idx]) / (self.max[idx] - self.min[idx]) #对应位置做最大最小化
...
def preprocess(datadir, outdir, continous_features, categorial_features):#预处理
    ...
    dists.build(os.path.join(datadir, 'fe_add_train_data.csv'), continous_features)#得到了最大最小值
            ...
            with open(os.path.join(datadir, 'fe_add_train_data.csv'), 'r') as f:
                for line in f:
                    ...
                    for i in range(0, len(continous_features)): #连续型两个列表，一个ffm，一个神经网络
                        ...
                        val = dists.gen(i, features[continous_features[i]]) #实际位置，该行实际值，最大最小化
