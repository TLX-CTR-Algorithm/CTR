...
class Model():
    ...
    def embeding(self,embed_inputs):
        with tf.name_scope("embedding"):
            ...
            print ('oridata_dim:{}'.format(oridata_dim))
    ...
    def construction(self, inputs,keep_prob):
        ...
        net = slim.fully_connected(net, self.outunits[0], activation_fn=None, keep_prob=keep_prob, scope=end_point)
        end_points[end_point] = net

        end_point = 'fully_con2'
        net = slim.fully_connected(net, self.outunits[1], activation_fn=None, keep_prob=keep_prob, scope=end_point)
        end_points[end_point] = net

        end_point = 'logits'
        logits = slim.fully_connected(net, self.outunits[2], activation_fn=None, keep_prob=keep_prob, scope=end_point)
